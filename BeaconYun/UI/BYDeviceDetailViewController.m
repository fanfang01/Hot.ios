...
//<BDSClientASRDelegate,BDSClientWakeupDelegate>
...
- (void)initData {
    if (!_dataArray) {
        _dataArray = [NSMutableArray arrayWithObjects:@{@"title":@"模式",@"command":@"0x03"},
                      @{@"title":@"强度加",@"command":@"0x04"},
                      @{@"title":@"强度减",@"command":@"0x05"},
...
- (AdvertiseView *)advertiseView
{
    if (!_advertiseView) {
        ...
        _advertiseView.buttonBlock = ^(NSInteger index) {
            ...
            MinewModule *module = strongSelf.api.lastModule;
            NSString *commandStr = strongSelf.dataArray[index][@"command"];
            
            NSData *data = [strongSelf hexString:commandStr];
